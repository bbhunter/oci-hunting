#### Description #################################################################################
#
# Indexes all IAM policies in an OCI environement, which contain statements for individual users or 'any-user'.
#
####
...
echo "-----"
echo "The following IAM policies are assigned to individual users"
...
do
    ...
    iam_policy_user_statements=$(oci iam policy get --policy-id "$iam_policy_id" | jq -r '.data | ."statements"[]' | grep -e 'allow any-user' -e 'allow user')

    if [[ "$iam_policy_user_statements" ]]
    then
        ...
        echo "$iam_policy_user_statements" | sed "s/ allow/\nAllow/gi"
        echo ""
